...
def main(argv):
    ...
    parser.add_argument("-v", "--verbose", action="store_true", dest="verbosity", help="Print verbose information for debugging", default=None)
    parser.add_argument("-q", "--quiet", action="store_false", dest="verbosity", help="Suppress warnings", default=None)
    parser.add_argument("-p", "--pixels", action="store_false", dest="contiguous", help="Generate a separate shape for each pixel; do not group pixels into contiguous areas of the same colour", default=True)
    parser.add_argument("-u", "--opaque", action="store_true", dest="opaque", help="Opaque only; do not create shapes for fully transparent pixels. ", default=None)
    parser.add_argument("-1", "--one", action="store_true", dest="keep_every_point", help="1-pixel-width edges on contiguous shapes; default is to remove intermediate points on straight line edges. ", default=None)
    ...
    for file_path in all_files:
        ...
        out_img = '{}/{}.svg'.format(args.out,str(parts[-1:][0][:-4]))  
        print('copy src:{} dest:{}'.format(file_path,out_img))

        svg_info = png_to_svg(file_path, contiguous=args.contiguous, opaque=args.opaque, keep_every_point=args.keep_every_point)

        text_file = open(out_img, "w")
        n = text_file.write(svg_info)
        text_file.close()
